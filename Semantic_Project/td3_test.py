...
# ------------------------------------------------------------------------------------------------------------------ #
# --------------------------------------------------------------
agent = Agent_TD3(alpha, beta, n_agent*n_state, tau, gamma, n_actions, memory_size, fc1_dims, fc2_dims, fc3_dims, fc4_dims, fc5_dims, batch_size, 2, 'OU')
    ...
    for i_step in range(n_step):
        ...
        for i_agent in range(n_agent):
            ...
            if action[i_agent,0] == 0 : #Allocate to Micro
                ...
                action[i_agent, n_RB + n_RB + 1 + 1] = np.round(np.clip(action_choose[0, n_RB + n_RB + 1 + 1 + (i_agent * n_action)] * 20, 1, 20))
                #symbol_lenghth[i_agent]= action[i_agent, n_RB + n_RB + 1 + 1]
                #symbol_lenghth[i_agent] = np.random.randint(1, 20)
                symbol_lenghth[i_agent] = 10
                # 边界检查
        ...
        for i_agent in range(n_agent):
            ...
            semantic_rate_level[i_agent, i_step] = (semantic_similarity[i_agent] / symbol_lenghth[i_agent]) * BW *duty[i_agent]*1000#khz
            ...
            semantic_WiFi_HSSE[i_agent, i_step] = semantic_WiFi_rate[i_agent, i_step] / symbol_lenghth[i_agent]
...
for i_episode in range(n_episode):
    ...
    semantic_HSSE_mean_all[i_episode] = np.mean(semantic_HSSE_per_episode[:, i_episode])
    semantic_WiFi_HSSE_mean_all[i_episode] = np.mean(semantic_WiFi_HSSE_per_episode[:, i_episode])

semantic_HSSE_mean_all_avg = np.mean(semantic_HSSE_mean_all)
semantic_WiFi_HSSE_mean_all_avg = np.mean(semantic_WiFi_HSSE_mean_all)
semantic_rate_level_mean_all_avg = np.mean(semantic_rate_level_mean_all)
semantic_WiFi_level_mean_all_avg = np.mean(semantic_WiFi_level_mean_all)

print(semantic_HSSE_mean_all_avg)
print(semantic_WiFi_HSSE_mean_all_avg)
print(semantic_rate_level_mean_all_avg)
print(semantic_WiFi_level_mean_all_avg)

average_every_vehicle_user_per_row = np.mean(semantic_rate_level_per_episode, axis=1, keepdims=True) / 1000
average_every_WiFi_user_per_row = np.mean(semantic_WiFi_level_per_episode, axis=1, keepdims=True)
# 计算平方和 分母
squared_sum_vehicle = np.sum(average_every_vehicle_user_per_row ** 2)
squared_sum_WiFi = np.sum(average_every_WiFi_user_per_row ** 2)
squared_sum_total = squared_sum_vehicle + squared_sum_WiFi
# 计算和并进行平方 分子
sum_vehicle = np.sum(average_every_vehicle_user_per_row)
sum_WiFi = np.sum(average_every_WiFi_user_per_row)
sum_squared_total = (sum_vehicle + sum_WiFi) ** 2
# 求公平性
fairness = sum_squared_total / (squared_sum_total * (n_agent + n_agent))
print(fairness)